...
- **Open schema question before seeding UNILAG**: `Course.catchmentCutOff` is currently a single value, but UNILAG's official cut-off publication gives a *different* catchment cut-off per catchment state (Ekiti/Lagos/Ogun/Ondo/Osun/Oyo), not one number per course — see the dossier's UNILAG section for the finding and example figures. Decide before writing the seed script whether to collapse to one value (e.g. the lowest of the six) or extend the schema to a per-state cut-off; retrofitting after seeding means re-touching every UNILAG course row.
...
3. **Scoring** — `POST /scoring/aggregate`. Apply the university's `ScoringPolicy` weightings to UTME/Post-UTME/O-Level components, compare against the course's merit/catchment/ELDS cut-offs. Return `AggregateScoreResult` with a `breakdown` per component — this is what makes the result screen's explanation legible, don't skip fields.
   - **O-Level component**: grade the course's *required* subject combination (`AdmissionRequirement.requiredOLevelSubjects`, per the candidate's stream — science/arts/commercial), not the candidate's 5 highest-graded credits overall. "Best 5" was an earlier, incorrect assumption — see the correction note in `docs/jamb-data-dossier.md`'s UNILAG section for the full per-stream breakdown and which universities weight O-Level into the aggregate at all (UNILAG, OAU, FUTA, FUNAAB, FUOYE; UI uses O-Level only as an eligibility gate, not in the number).
   - **Grade-point scale is per-university, not universal** — don't hardcode one A1→C6 point table. UNILAG: A1=4.0…C6=2.0 (÷5, averaged). OAU: A1=10…C6=5 (÷5). FUOYE: A1=6…C6=1. FUNAAB: A1=6…C6=1, summed (max 30) then ×(5/3) — **Confirmed** directly off FUNAAB's own Help Desk (the only Confirmed formula of the six). Model this as part of `ScoringPolicy` (or a related per-university lookup), not a shared constant.
   - **OAU's formula** (`50% JAMB÷8 + 40% Post-UTME raw score (out of 40) + 10% O'Level`) is community-sourced (Facebook admissions groups, aggregator sites), not read off an OAU official page — same Likely confidence tier as the other four (non-FUNAAB) universities' formulas, not Confirmed.
   - **FUNAAB has no Post-UTME/screening term at all** — it's a straight 50/50 UTME/O-Level split (`UTME÷8` + `O'Level points×5/3`). This overturns what was previously assumed (a 50/20/30 JAMB/O-Level/screening split) — don't build a screening-score input for FUNAAB's formula. Two edge cases to implement explicitly, not silently: a 1-point deduction when a candidate presents two O'Level sittings/exam boards, and Agriculture-in-lieu-of-Biology counting toward eligibility but contributing zero O'Level points.
